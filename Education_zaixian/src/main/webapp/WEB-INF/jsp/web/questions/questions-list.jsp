				...
				<div class="fl col-7">
					<section class="mr30 pt10">
						<section class="c-infor-tabTitle c-tab-title">
						    <a href="/front/questions/list/0" title="全部问答" onclick="submitForm(0,'type')" <c:if test="${questions.type==0}">class="current"</c:if>>全部问答</a>
							<a href="/front/questions/list/1" title="课程问答" onclick="submitForm(1,'type')" <c:if test="${questions.type==1}">class="current"</c:if>>课程问答</a>
							<a href="/front/questions/list/2" title="学习分享" onclick="submitForm(2,'type')" <c:if test="${questions.type==2}">class="current"</c:if>>学习分享</a>
						</section>
						...
						<div class="js-wrap">
							...
							<section class="fl">
								<ol class="js-tap clearfix">
									<li <c:if test="${questions.orderFalg=='addTime' }">class="current bg-orange"</c:if>><a onclick="submitForm('addTime','order')" href="/front/questions/list/3" title="最新">最新</a></li>
									<li <c:if test="${questions.orderFalg=='replycount' }">class="current bg-orange"</c:if>><a onclick="submitForm('replycount','order')" href="/front/questions/list/4" title="热门">热门</a></li>
									<li <c:if test="${questions.orderFalg=='status0' }">class="current bg-orange"</c:if>><a onclick="submitForm('status0','order')" href="/front/questions/list/5" title="等待回答">等待回答</a></li>
								</ol>
							</section>
						</div>
						<!-- /问题列表 开始 -->
										...
										<c:forEach items="${questionslist }" var="question">
											<li>
												<aside class="q-head-pic">
													<c:choose>
														<c:when test="${not empty questione.eduUser.pic_img }">
															<img src="<%=staticImage %>${questione.eduUser.pic_img }" alt="">
														</c:when>
														<c:otherwise>
													...
													<p class="hLh30 txtOf">
														<span class="c-999"> <c:if test="${empty questione.eduUser.show_name }">${questione.eduUser.email }</c:if> <c:if test="${not empty questione.eduUser.show_name }">${questione.eduUser.show_name }</c:if>
														</span>
													</p>
												</aside>
												<section class="q-txt-box">
													...
													<h3 class="hLh30 txtOf mt5">
														...
														<c:if test="${not empty question.content }">
															<c:if test="${question.status==0 }">
																<span class="fsize12 c-999 vam"> <tt class="c-ccc f-fM mr5">[最新回答]</tt> <c:forEach items="${question.content }" var="questionsComment">
															...
															<c:if test="${question.status==1 }">
																...
																<c:forEach items="${question.content }" var="questionsComment">
																	<c:out value=""></c:out>
																	</c:forEach>
																</span>
																<!-- 采纳最佳显示最佳答案内容 -->
													...
													<div class="mt15">
														...
														<fmt:formatDate value="${question.add_time }" type="date" pattern="yyyy-MM-dd HH:mm:ss"/>
														</span> 
						...
						<section class="pt20">
							<div class="taglist clearfix">
								...
								<c:forEach items="${listTag }" var="questionsTag">
									<a title="${questionsTag.questions_tag_name }" data-id="${questionsTag.questions_tag_id }" class="list-tag <c:if test='${questionsTag.questions_tag_id==questions.questionsTagId }' >onactive</c:if>" href="javascript:;" onclick="submitForm('${questionsTag.questions_tag_id }','questionsTagId')">${questionsTag.questions_tag_name }</a>
								</c:forEach>
							</div>
						</section>
						<!-- /标签云 -->
