	...
	<div class="good-anwer-box">
		...
		<section class="good-answer mt10">
			<section class="question-list lh-bj-list pr">
				<ul class="pr10">
					<c:forEach items="${comments2}" var="questionsComment">
						<li>
							...
							<div class="of mt5">
								<span class="fr"><font class="fsize12 c-999 ml5"><fmt:formatDate
											type="both" value="${questionsComment.add_time }"
											pattern="yyyy-MM-dd HH:mm" /></font></span> <span class="fl"> <a
									href="javascript: void(0)" title="回答" class="noter-dy vam"
									onclick="getCommentById(this,${questionsComment.id })"> <em
										class="icon18">&nbsp;</em>全部评论(<span>${questionsComment.reply_count }</span>)
								</a> <tt class="noter-zan vam ml10 f-fM" title="赞一下"
										onclick="addPraise(${questionsComment.id },2,this)">
...
<div class="q-i-noter-box">
		...
		<li class="unBr">
			...
			<div class="of">
				<section class="n-reply-wrap">
					<fieldset>
						<textarea placeholder="输入您要回复的文字" name="questionsComment.content"
							onkeyup="$(this).parent().next().find('tt').html('');"></textarea>
					</fieldset>
					<p class="of mt5 tar pl10 pr10">
						<span class="fl"><tt class="c-red"></tt></span> <u
							class="hand mr10 qxBtn c-999" style="display: none;">取消</u> <a
							class="lh-reply-btn" title="回复" href="javascript: void(0)"
							onclick="addComment(this)">回复</a>
					</p>
				</section>
			</div>
		</li>
	...
	<c:if test="${not empty comments1  }">
		<div class="q-n-r-box">
			<section class="question-list lh-bj-list pr">
				<ul class="pr10">
					<c:forEach items="${comments1}" var="questionsComment"
						varStatus="index">
						<li>
							<aside class="noter-pic">
								<c:choose>
							...
							<div class="of mt5">
								<span class="fr"><font class="fsize12 c-999 ml5">
										<!-- <a href="" class="c-blue mr10">删除</a> --> <fmt:formatDate
											type="both" value="${questionsComment.add_time}"
											pattern="yyyy-MM-dd HH:mm" />
								</font></span> <span class="fl"> <a href="javascript: void(0)"
									title="回答" class="noter-dy vam"
									onclick="getCommentById(this,${questionsComment.id})"> <em
										class="icon18">&nbsp;</em>(<span>${questionsComment.reply_count }</span>)
								</a> <tt class="noter-zan vam ml10" title="赞一下"
										onclick="addPraise(${questionsComment.id },2,this)">
							...
							<div class="n-reply" style="display: none; padding-left: 0;">
								<div>
									<section class="n-reply-wrap mt10">
										...
										<p class="of mt5 tar pl10 pr10">
											<span class="fl"><tt class="c-red"></tt></span><u
												class="hand mr10 qxBtn c-999">取消</u> <a
												href="javascript: void(0)" title="回复" class="lh-reply-btn"
												onclick="addReply(this)">回复</a>
										</p>
									</section>
									<input type="hidden" id="" name=""
